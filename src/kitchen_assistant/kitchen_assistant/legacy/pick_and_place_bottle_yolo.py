...
MODEL_PATH = resolve_model_path(resolve_model_path("yolo11n.pt"))
TARGET_NAME = "bottle"
...
PICK_PUSH_DIST  = 42                    # 최종 집기 시 대상 쪽으로 더 밀어주는 거리(기존 z - PICK_DZ 의미)
...
class ApplePickNode(Node):
    ...
    def pick_sequence(self, target_xyz, approach_axis, approach_sign):
        ...
        movel(posx([0.0, 0.0, 10.0, 0.0, 0.0, 0.0]), vel=VELOCITY, acc=ACC, 
        ...
        movel(posx([-100.0, 0.0, 50.0, 0.0, 0.0, 0.0]), vel=VELOCITY, acc=ACC, 
          ref=DR_BASE, mod=DR_MV_MOD_REL)
        wait(0.5)

        movej([-180.0, 0.0, 0.0, 0.0, -30.0, 0.0], vel=30, acc=30, 
          mod=DR_MV_MOD_REL)
        wait(0.5)
